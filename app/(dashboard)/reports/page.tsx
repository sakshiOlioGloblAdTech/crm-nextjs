...
const TABS: { value: ReportType; label: string; icon: any; color: string }[] = [
  { value: "sales",     label: "Sales Report",       icon: TrendingUp, color: "text-brand-600"   },
...
export default function ReportsPage() {
  ...
  const INPUT = "px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 bg-surface";
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      {/* ── Tabs + Date filters ── */}
      <div className="bg-surface border border-gray-200 rounded-2xl p-4 flex flex-wrap items-center gap-4">
      ...
      {!loading && data?.type === "sales" && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Total Revenue",   value: formatINR(data.summary.totalRevenue),  icon: IndianRupee,  bg: "bg-brand-50",   ic: "text-brand-600"   },
              ...
              const Icon = c.icon;
              return (
                <div key={c.label} className="bg-surface border border-gray-200 rounded-2xl p-5">
                    ...
                    <div key={mode}>
                      <div className="flex justify-between items-center mb-1.5">
                        <span className={`text-xs font-bold px-2.5 py-1 rounded-lg ${
                          mode === "COD" ? "bg-orange-100 text-orange-700" : "bg-brand-100 text-brand-700"
                        }`}>{mode}</span>
                        <span className="text-sm font-bold text-gray-900">{count} orders ({pct}%)</span>
                      </div>
                      <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${mode === "COD" ? "bg-orange-400" : "bg-brand-500"}`}
                          style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  );
                ...
                <tbody className="divide-y divide-gray-50">
                  ...
                  {data.orders.map((o: any) => (
                    <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-brand-600 font-semibold whitespace-nowrap">{o.orderNumber}</td>
